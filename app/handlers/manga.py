...
async def manga_search_handler(message: Message, state: FSMContext):
    ...
    async for db in get_db():
        ...
        try:
            ...
            # Сохраняем маппинг idx -> real_id в state
...
async def manga_card_callback(callback: CallbackQuery, state: FSMContext):
    ...
    async for db in get_db():
        ...
        try:
            ...
            # Отправляем с обложкой, если есть
            if manga.cover_url:
                try:
                    ...
                    # Если не удалось отправить с обложкой, отправляем текстом
