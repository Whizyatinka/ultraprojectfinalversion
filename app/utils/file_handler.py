...
class FileHandler:
    ...
    async def prepare_for_telegram(self, filepath: str) -> tuple[InputFile, bool]:
        ...
        if file_size > config.MAX_FILE_SIZE_TG:
            logger.warning(f"File {filepath} exceeds 50MB, compression needed")
            compressed_path = await self.compress_image(filepath)
            new_size = self.check_file_size(compressed_path)
            
            if new_size > config.MAX_FILE_SIZE_TG:
                logger.error(f"File still too large after compression: {new_size}")
